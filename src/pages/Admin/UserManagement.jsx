...
const TableRowSkeleton = ({ isBulkMode, isMobile, isTablet, role_id }) => {
  ...
  return (
    <TableRow>
      ...
      {/* Sr. No. skeleton */}
      <TableCell>
        <Skeleton variant="text" width={28} height={18} sx={{ bgcolor: alpha(theme.palette.primary.main, 0.2) }} />
      </TableCell>
      <TableCell>
...
const UserCard = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Sr. No. badge — top-left (shifts right when bulk checkbox is visible) */}
        <Box
          sx={{
            position: 'absolute',
            top: 8,
            left: isBulkMode ? 36 : 8,
            zIndex: 1,
          }}
        >
          <Chip
        ...
        <CardContent sx={{ p: { xs: 1.5, sm: 1.75 }, pt: { xs: 3, sm: 3.5 } }}>
          {/* User Info Row */}
            ...
            {(role_id === 1 || role_id === 3) && (
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 0.75 }}>
                <Typography
                  variant="caption"
                  color="text.secondary"
                  sx={{ fontSize: { xs: '0.55rem', sm: '0.6rem' }, fontWeight: 500, letterSpacing: 0.3, display: 'flex', alignItems: 'center', gap: 0.5, cursor: 'pointer' }}
                  onClick={() => setShowShiftName(!showShiftName)}
                >
                  {showShiftName ? "Shift Name" : "Shift Time"}
                  <SwapHorizIcon sx={{ fontSize: 12, color: theme.palette.primary.main }} />
                </Typography>
                <Typography
                  variant="caption"
                  fontWeight={600}
                  color={user.shift ? "text.primary" : "error.main"}
                  sx={{ fontSize: { xs: '0.55rem', sm: '0.6rem' }, letterSpacing: 0.2 }}
                >
                  {user.shift ? (showShiftName ? user.shift.shiftName : `${formatTo12Hour(user.shift.shiftStartTime)} - ${formatTo12Hour(user.shift.shiftEndTime)}`) : "Free Time"}
                </Typography>
              </Box>
            )}
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 0.75 }}>
...
const ResponsiveTable = ({
  ...
  const SrNoHeader = () => (
    ...
    <TableCell
      ...
      sx={{
        ...
        width: '60px',
        p: 0,  // Remove padding
        py: { xs: 0.5, sm: 0.75 },  // Minimal vertical padding for readability
      }}
    >
      Sr. No.
    </TableCell>
          ...
          <TableHead>
            <TableRow>
              ...
              {isSuperAdmin ? (
                ...
                <>
                  {(role_id === 1 || role_id === 3) && <TableCell>{showShiftName ? "Shift Name" : "Shift Time"}</TableCell>}
  ...
  const getColSpan = () => {
    ...
    cols += 2; // Profile & Name
    cols += 2; // Email
    if (isSuperAdmin || role_id === 1 || role_id === 3) cols += 1; // Mobile No
    if (isSuperAdmin) {
      cols += 2; // Plan Start + Plan Expiry
    } else {
      cols += 1; // Last Check-in
      if (role_id === 1 || role_id === 3) cols += 1; // Shift Time
    }
    cols += 2; // Actions
    return cols;
  };
            ...
            <TableRow>
              ...
              {isSuperAdmin ? (
                ...
                <>
                  {(role_id === 1 || role_id === 3) && (
                    <TableCell sx={{ fontWeight: 600, fontSize: { xs: '0.7rem', sm: '0.75rem', md: '0.8rem' }, color: theme.palette.primary.main }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, cursor: 'pointer' }} onClick={() => setShowShiftName(!showShiftName)}>
                        {showShiftName ? "Shift Name" : "Shift Time"}
                        <SwapHorizIcon fontSize="small" sx={{ color: theme.palette.primary.main, fontSize: 14 }} />
                      </Box>
                    </TableCell>
                  )}
                  <TableCell sx={{ fontWeight: 600, fontSize: { xs: '0.7rem', sm: '0.75rem', md: '0.8rem' }, color: theme.palette.primary.main }}>
        ...
        <TableBody>
          <AnimatePresence>
            {(users || []).map((user, index) => {
              ...
              // Sr. No. = page offset + 1-based index
                  ...
                  {isSuperAdmin ? (
                    ...
                    <>
                      {(role_id === 1 || role_id === 3) && (
                        <TableCell sx={{
                          fontSize: { xs: '0.55rem', sm: '0.6rem', md: '0.75rem' },
                          color: user.shift ? 'text.secondary' : 'error.main',
                          fontWeight: user.shift ? 400 : 500
                        }}>
                          {user.shift ? (showShiftName ? user.shift.shiftName : `${formatTo12Hour(user.shift.shiftStartTime)} - ${formatTo12Hour(user.shift.shiftEndTime)}`) : "Free Time"}
                        </TableCell>
                      )}
...
const UserManagement = () => {
  ...
  const [showShiftName, setShowShiftName] = useState(false);
  ...
  const handleDownloadPDF = async () => {
    ...
    const tableColumn = role_id === 2
      ? ["Sr. No.", "Name", "Email", "Mobile No", "Status", "Joined Date"]
      : ["Sr. No.", "Name", "Email", "Status", "Joined Date"];

    const tableRows = currentDisplayUsers.map((user, index) => {
      if (role_id === 2) {
        return [
          index + 1,
          user.name || "N/A",
        ...
        {viewMode === 'table' && (
          <Box sx={{ p: { xs: 1, sm: 1.5 } }}>
            ...
            <ResponsiveTable
              ...
              setShowShiftName={setShowShiftName}
            />
          ...
          <Box sx={{ p: { xs: 1, sm: 1.5 } }}>
            ...
            {isLoading ? (
              ...
              <>
                ...
                {/* Rows per page selector for card view */}
