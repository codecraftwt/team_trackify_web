...
const AddUser = ({ open, onClose, editingUser = null }) => {
  ...
  const getNameFieldLabel = () => {
    if (isSuperAdmin && !editingUser) return "Organization Name";
    return "User Name";
  };
    ...
    switch (name) {
      ...
      case "password":
        if (!editingUser) {
          ...
          else if (value.length > 20) error = "Password must be less than 20 characters";
          else if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(value))
  ...
  const handleBlur = (e) => {
    ...
    const fieldValue = name === "mobile" ? value.replace(/[^0-9]/g, "") : value;
    const error = validateField(name, fieldValue);
    setErrors((prev) => ({ ...prev, [name]: error }));
  };
  ...
  const handleImageChange = (e) => {
    ...
    const allowedTypes = ["image/jpeg", "image/jpg", "image/png", "image/gif"];
    if (!allowedTypes.includes(file.type)) {
      setErrors((prev) => ({ ...prev, avtar: "Only JPG, PNG, and GIF images are allowed" }));
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setErrors((prev) => ({ ...prev, avtar: "File size should be less than 5MB" }));
      return;
    }
    setErrors((prev) => ({ ...prev, avtar: "" }));
  ...
  const removeImage = () => {
    setFormData((prev) => ({ ...prev, avtar: null }));
    setPreviewImage(null);
    setImageRemoved(true);
    setErrors((prev) => ({ ...prev, avtar: "" }));
  };
...
const handleSubmit = async (e) => {
  ...
  const getDialogProps = () => {
    ...
    return { fullScreen: false, maxWidth: "sm" };
  };
