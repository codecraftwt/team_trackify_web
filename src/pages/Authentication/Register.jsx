...
const Register = () => {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (verifyEmailOTP.fulfilled.match(resultAction)) {
        ...
        // console.log("🎯 Plan to pass to login:", planToPass);
        ...
        setTimeout(() => {
          navigate('/login', {
            state: {
              selectedPlan: planToPass,
              fromPricing: !!planToPass,
              fromRegistration: true,
              message: planToPass
                ? `Registration successful! Please login to purchase the ${planToPass.name} plan.`
  ...
  // Clear plan from session storage when component unmounts if not used
  useEffect(() => {
    return () => {
