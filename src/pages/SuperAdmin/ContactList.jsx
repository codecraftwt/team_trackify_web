...
//         <Tooltip title="Send Email (Opens Gmail)">
//           <IconButton
//             size="small"
//             onClick={(e) => {
//               e.stopPropagation();
...
const StatusFilter = ({ selectedStatus, onStatusChange }) => {
  ...
  const statuses = [
    { value: "all", label: "All Status", color: theme.palette.info.main },
    { value: "pending", label: "Pending", color: theme.palette.warning.main },
    { value: "contacted", label: "Contacted", color: theme.palette.primary.main },
    { value: "replied", label: "Replied", color: theme.palette.success.main },
  ];
    ...
    <>
      <Button
        ...
        sx={{
          ...
          fontSize: { xs: '0.7rem', sm: '0.75rem' },
          borderColor: alpha(theme.palette.primary.main, 0.3),
          color: selectedStatus !== "all" ? 'white' : theme.palette.primary.main,
          bgcolor: selectedStatus !== "all" ? theme.palette.primary.main : 'transparent',
          '&:hover': {
            borderColor: theme.palette.primary.main,
            bgcolor: selectedStatus !== "all" ? theme.palette.primary.dark : alpha(theme.palette.primary.main, 0.05),
          },
        }}
      >
        {getSelectedLabel()}
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, width: '100%' }}>
                ...
                {selectedStatus === status.value && (
                  <Typography variant="caption" sx={{ ml: 'auto', fontSize: '0.7rem' }}>
                    ✓
                  </Typography>
                )}
              </Box>
...
const ContactList = () => {
  ...
  // Updated filtered contacts with status filter
  const filteredContacts = contacts.filter(contact =>
    (contact.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    contact.email?.toLowerCase().includes(searchQuery.toLowerCase())) &&
