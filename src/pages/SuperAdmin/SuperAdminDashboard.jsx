...
const SuperAdminDashboard = () => {
  ...
  // Stats Cards Component - Constant Height
  const StatsCards = () => {
    const itemVariants = {
      hidden: { opacity: 0, y: 20 },
      visible: {
    ...
    const getGridColumns = () => {
      if (isSmallMobile) return 12; // 1 card
      if (isMobile) return 12; // 1 card
      if (isTablet) return 6; // 2 cards
      return 3; // 4 cards
    };
  ...
  // Revenue Card Component - Constant Height
  const RevenueCard = () => {
    return (
      <Paper
        elevation={0}
      ...
      {isRefreshing && (
        ...
        >
          <Box sx={{ textAlign: 'center' }}>
            ...
            />
            <Typography variant="body1" color="#0f766e" fontWeight={500}>
              Refreshing data...
            </Typography>
          ...
          <motion.div variants={itemVariants}>
            ...
            >
              <Typography 
                ...
                fontWeight="700" 
                color="#0f766e"
                sx={{ fontSize: isSmallMobile ? '1.2rem' : isMobile ? '1.3rem' : isTablet ? '1.4rem' : '1.5rem' }}
              >
                Super Admin Dashboard
              </Typography>
              <IconButton onClick={refreshData} disabled={isRefreshing}>
                <RefreshIcon sx={{ color: '#0f766e', animation: isRefreshing ? 'spin 1s linear infinite' : 'none' }} />
              </IconButton>
            </Box>
          </motion.div>
