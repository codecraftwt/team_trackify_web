...
// NEW: Set/Create Base Price (Superadmin only)
export const setBasePrice = createAsyncThunk(
  "plan/setBasePrice",
  async (basePricePerUserPerMonth, { rejectWithValue }) => {
    try {
      const response = await api.post("/plans/base-price", { basePricePerUserPerMonth });
      toast.success(response.data?.message || "Base price created successfully!");
      return response.data;
    } catch (error) {
      console.error("Error setting base price:", error);
      const errorMessage = error.response?.data?.message || "Failed to set base price";
      toast.error(errorMessage);
      return rejectWithValue(error.response?.data || errorMessage);
    }
  }
...
// NEW: Update Base Price (Superadmin only)
export const updateBasePrice = createAsyncThunk(
  "plan/updateBasePrice",
  async (basePricePerUserPerMonth, { rejectWithValue }) => {
    try {
      const response = await api.put("/plans/base-price", { basePricePerUserPerMonth });
      toast.success(response.data?.message || "Base price updated successfully!");
      return response.data;
    } catch (error) {
      console.error("Error updating base price:", error);
      const errorMessage = error.response?.data?.message || "Failed to update base price";
      toast.error(errorMessage);
      return rejectWithValue(error.response?.data || errorMessage);
    }
  }
...
// NEW: Get Price History (Current + Previous price)
export const getPriceHistory = createAsyncThunk(
  "plan/getPriceHistory",
  async (_, { rejectWithValue }) => {
    try {
      const response = await api.get("/plans/base-price/history");
      return response.data;
    } catch (error) {
...
const planSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(setBasePrice.fulfilled, (state, action) => {
        state.loading = false;
        state.basePrice = action.payload?.data?.currentPrice || action.payload?.currentPrice;
      })
      .addCase(setBasePrice.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
      ...
      .addCase(updateBasePrice.fulfilled, (state, action) => {
        state.loading = false;
        state.basePrice = action.payload?.data?.currentPrice || action.payload?.currentPrice;
        state.previousPrice = action.payload?.data?.previousPrice || action.payload?.previousPrice;
      })
      ...
      .addCase(getPriceHistory.fulfilled, (state, action) => {
        ...
        state.basePrice = action.payload?.currentPrice;
        state.previousPrice = action.payload?.previousPrice;
      })
