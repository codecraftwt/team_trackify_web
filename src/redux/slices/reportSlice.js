...
// NEW THUNK: Get user reports by admin ID (using TrackingSession)
export const getUserReportsByAdminId = createAsyncThunk(
  "reports/getUserReportsByAdminId",
  async ({ adminId, fromDate, toDate, page = 1, limit = 10, search } = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await api.get(`/Tracking/admin/${adminId}/user-reports`, { params });
      return response.data;
    } catch (error) {
...
const reportSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      // NEW: Get User Reports by Admin ID
      .addCase(getUserReportsByAdminId.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(getUserReportsByAdminId.fulfilled, (state, action) => {
        state.loading = false;
        state.reports = action.payload.data || [];
        state.pagination = {
          page: action.payload.pagination?.page || 1,
